...
def cookSoup(soup: BeautifulSoup) -> list:
    ...
    joblist = list(
        soup.find(id="mosaic-provider-jobcards")
...
if __name__ == "__main__":
    ...
    chromeDriverPath = "/mnt/d/scraping/driver/chromedriver.exe"
    options = Options()
    ...
    with webdriver.Chrome(service=service, options=options) as driver:
        for start in range(740, 800, 10):
            url = (
                "https://ca.indeed.com/jobs?q=developer&l=Canada&sort=date&filter=0"
                + f"&start={start}"
            )
